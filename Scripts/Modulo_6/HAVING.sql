...
SELECT id_estacion, 
       COUNT(id_estacion) AS conteo_estaciones
FROM mantenimiento
WHERE fecha BETWEEN '2025-01-01' AND '2025-12-31'
GROUP BY id_estacion
...
SELECT id_pais, 
       AVG(superficie) AS superficie_promedio_por_pais
FROM ciudad
GROUP BY id_pais
...
SELECT id_estacion, 
       COUNT(id_estacion) AS conteo_de_registros
FROM registros_clima
GROUP BY id_estacion
...
-- Obtener las estaciones que hayan registrado una humedad máxima mayor al 90%
SELECT id_estacion, 
       MAX(humedad) AS humedad_maxima
FROM registros_clima
GROUP BY id_estacion
HAVING humedad_maxima > 90;
